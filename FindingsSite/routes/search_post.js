        ...
        if (err) {
            ...
            if (req.session.login != true) {
                ...
                client.query("SELECT start_no, end_no FROM diag_category WHERE id=\'" + escapeStr(req.body.organ) + "\'", function (err, result) {
                    ...
                    client.query("SELECT no, name_jp FROM diag_name WHERE no >= " + result.rows[0].start_no + " AND no <= " + result.rows[0].end_no + " ORDER BY diag_order, no", function (err, result) {
                        done();
                ...
                var SQL = "SELECT exam_id, to_char(exam_day,\'yyyy/mm/dd\') AS e_day, exam.pt_id AS p_id, pt_name, exam_type.name_jp AS type_name " + "FROM exam, patient, exam_type ";
                var criteria = "WHERE exam.pt_id=patient.pt_id AND exam.exam_type=exam_type.type_no ";
...
//# sourceMappingURL=search_post.js.map
